...
class SuratKeluarModel extends CI_Model {
    ...
    public function getall($penID) {
        if($this->session->userdata('masuk_level')=='KASUBAGUMUM' || $this->session->userdata('masuk_level')=='PIMPINAN'){
            $query = $this->db->query('SELECT * FROM surat WHERE surKategori=? ORDER BY surTglTerimaKeluar DESC',array('K'));
            return $query->result();
        }else{    
            $query = $this->db->query('SELECT * FROM surat WHERE surKategori=? AND surID IN (SELECT disSumSukID FROM disposisi WHERE (disDariPenID=? OR disKepadaPenID=?)) ORDER BY surTglTerimaKeluar DESC',array('K',$penID,$penID));
            return $query->result();
        }
    }
    //oke,oke
    ...
    public function getall_tabayun($penID) {
        if($this->session->userdata('masuk_level')=='KASUBAGUMUM' || $this->session->userdata('masuk_level')=='PIMPINAN'){
            $query = $this->db->query('SELECT * FROM surat s,perkara p WHERE s.surID=p.perSurID AND s.surKategori=? AND s.surJenis=? AND s.surTabayun=? ORDER BY s.surTglTerimaKeluar DESC',array('K','PENTING','Y'));
            return $query->result();
        }else{
            $query = $this->db->query('SELECT * FROM surat s,perkara p WHERE s.surID=p.perSurID AND s.surKategori=? AND s.surJenis=? AND s.surTabayun=?  AND s.surID IN (SELECT disSumSukID FROM disposisi WHERE (disDariPenID=? OR disKepadaPenID=?))  ORDER BY s.surTglTerimaKeluar DESC',array('K','PENTING','Y',$penID,$penID));
            return $query->result();
        }
    }
    //oke
    public function getall_penting($penID) {
        if($this->session->userdata('masuk_level')=='KASUBAGUMUM' || $this->session->userdata('masuk_level')=='PIMPINAN'){
            $query = $this->db->query('SELECT * FROM surat s,telaah_staff t WHERE s.surID=t.telSumID AND s.surKategori=? AND s.surJenis=? AND s.surTabayun=? ORDER BY s.surTglTerimaKeluar DESC',array('K','PENTING','N'));
            return $query->result();
        }  else {
            $query = $this->db->query('SELECT * FROM surat s,telaah_staff t WHERE s.surID=t.telSumID AND s.surKategori=? AND s.surJenis=? AND s.surTabayun=? AND s.surID IN (SELECT disSumSukID FROM disposisi WHERE (disDariPenID=? OR disKepadaPenID=?)) ORDER BY s.surTglTerimaKeluar DESC',array('K','PENTING','N',$penID,$penID));
            return $query->result();
        }
    }
    //oke
    public function getall_rabi($penID,$surJenis) {
        if($this->session->userdata('masuk_level')=='KASUBAGUMUM' || $this->session->userdata('masuk_level')=='PIMPINAN'){
            $query = $this->db->query('SELECT * FROM surat WHERE surKategori=? AND surJenis=? AND surTabayun=? ORDER BY surTglTerimaKeluar DESC',array('K',$surJenis,'N'));
            return $query->result();
        }
    }
    //oke
    ...
    //oke
    public function getall_suratmasuk() {
        $query=  $this->db->query('SELECT * FROM surat WHERE surKategori=?',array('M'));
         return $query->result();
    }
    //oke
