...
if ($data['kategori'] == 'MASUK') {
    if ($data['periode'] == 'prh') {
        ...
        $where = array('M', 'BIASA', $ci->Konfigurasi->dateToMysql($data['tglawal']), $ci->Konfigurasi->dateToMysql($data['tglakhir']));
        ...
        $sql = 'SELECT * FROM surat WHERE surKategori=? AND surJenis=? AND surTglTerimaKeluar>=? AND surTglTerimaKeluar<=? ORDER BY surTglTerimaKeluar ASC';
        $where = array('M', 'BIASA', $data['thnawal'] . '-' . $data['blnawal'] . '-01', $data['thnakhir'] . '-' . $data['blnakhir'] . '-' . $ci->Konfigurasi->getJmlHari($data['blnakhir'], $data['thnakhir']));
        ...
        $where = array('M', 'BIASA', $data['thnawal2'], $data['thnakhir2']);
    ...
    if ($data['periode'] == 'prh') {
        $sql = 'SELECT * FROM surat WHERE surKategori=? AND surJenis=? AND surTglTerimaKeluar >= ? AND surTglTerimaKeluar<=? ORDER BY surTglTerimaKeluar ASC';
        $where = array('K', 'BIASA', $ci->Konfigurasi->dateToMysql($data['tglawal']), $ci->Konfigurasi->dateToMysql($data['tglawal']));
        $sublabel = 'Periode ' . (($data['tglawal'] == $data['tglakhir']) ? $ci->Konfigurasi->MSQLDateToNormal($data['tglawal']) : $ci->Konfigurasi->MSQLDateToNormal($data['tglawal']) . ' - ' . $ci->Konfigurasi->MSQLDateToNormal($data['tglakhir']));
    } else if ($data['periode'] == 'prb') {
        $sql = 'SELECT * FROM surat WHERE surKategori=? AND surJenis=? AND surTglTerimaKeluar>=? AND surTglTerimaKeluar<=? ORDER BY surTglTerimaKeluar ASC';
        $where = array('K', 'BIASA', $data['thnawal'] . '-' . $data['blnawal'] . '-01', $data['thnakhir'] . '-' . $data['blnakhir'] . '-' . $ci->Konfigurasi->getJmlHari($data['blnakhir'], $data['thnakhir']));
        $sublabel = 'Periode ' . (($data['blnawal'] == $data['blnakhir'] && $data['thnawal'] == $data['thnakhir']) ? $ci->Konfigurasi->getNamaBulan($data['blnawal']) . ' ' . $data['thnawal'] : $ci->Konfigurasi->getNamaBulan($data['blnawal']) . ' ' . $data['thnawal'] . ' - ' . $ci->Konfigurasi->getNamaBulan($data['blnakhir']) . ' ' . $data['thnakhir']);
    } else if ($data['periode'] == 'prt') {
        $sql = 'SELECT * FROM surat WHERE surKategori=? AND surJenis=? AND YEAR(surTglTerimaKeluar)>=? AND YEAR(surTglTerimaKeluar)<=? ORDER BY surTglTerimaKeluar ASC';
        $where = array('K', 'BIASA', $data['thnawal2'], $data['thnakhir2']);
        $sublabel = 'Periode Tahun ' . (($data['thnawal2'] == $data['thnakhir2']) ? $data['thnawal2'] : $data['thnawal2'] . ' - ' . $data['thnakhir2']);
    }
}
?>
<div style="font-size: 14px; font-weight: bold;" align="center">Laporan Surat <?php echo(($data['kategori'] == 'MASUK') ? 'Masuk' : 'Keluar'); ?> (Jenis : Biasa)</div><br/>
...
<table align="center">
    <tr>
        ...
        <td class="th" valign="top" align="center" width="110">No. Surat</td>
        <td class="th" valign="top" align="center" width="260">Perihal</td>
        ...
        <td class="th" valign="top" align="center" width="60">Keamanan</td>
        <td class="th" valign="top" align="center" width="45">Sifat</td>
        <td class="th" valign="top" align="center" width="180">Asal</td>
        <td class="th" valign="top" align="center" width="180">Tujuan</td>
    </tr>
    <?php
    $surat = $ci->LaporanModel->getDataQuery($sql, $where);
    if (count($surat) < 1) {
        echo '<tr><td valign="top" align="left" colspan="10" style="font-size: 14px;"><i>Data Tidak Tersedia</i></td></tr>';
    } else {
        $no = 1;
    ...
    ?>
    <tr><td colspan="10" style="border-bottom: 0px;border-left: 0px;border-right: 0px;"></td></tr>  
    <tr><td colspan="10" style="border: 0px;"></td></tr>  
    <tr><td colspan="10" style="border: 0px;"></td></tr>  
    <tr><td colspan="10" style="border: 0px;"></td></tr>  
    <tr><td colspan="10" style="border: 0px;"></td></tr>
    <tr><td colspan="7" style="border: 0px;"></td><td style="border: 0px;" colspan="3">Pekanbaru, <?php echo $ci->Konfigurasi->MSQLDateToNormal(date('Y-m-d')); ?></td></tr>
    <tr><td colspan="7" style="border: 0px;"></td><td style="border: 0px;" colspan="3">Ketua Pengadilan Agama Pekanbaru,</td></tr>
    <tr><td colspan="10" style="border: 0px;"></td></tr>  
    <tr><td colspan="10" style="border: 0px;"></td></tr>  
    <tr><td colspan="10" style="border: 0px;"></td></tr>
    <tr><td colspan="7" style="border: 0px;"></td><td style="border: 0px;" colspan="3"><?php echo $ci->PegawaiModel->getbyid('58d76d351c75f')->pegNama; ?></td></tr>
</table>
