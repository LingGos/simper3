...
if ($data['kategori'] == 'MASUK') {
    if ($data['periode'] == 'prh') {
        ...
        $where = array('M', 'RAHASIA', $ci->Konfigurasi->dateToMysql($data['tglawal']), $ci->Konfigurasi->dateToMysql($data['tglakhir']));
        ...
        $sql = 'SELECT * FROM surat WHERE surKategori=? AND surJenis=? AND surTglTerimaKeluar>=? AND surTglTerimaKeluar<=? ORDER BY surTglTerimaKeluar ASC';
        $where = array('M', 'RAHASIA', $data['thnawal'] . '-' . $data['blnawal'] . '-01', $data['thnakhir'] . '-' . $data['blnakhir'] . '-' . $ci->Konfigurasi->getJmlHari($data['blnakhir'], $data['thnakhir']));
        ...
        $where = array('M', 'RAHASIA', $data['thnawal2'], $data['thnakhir2']);
    ...
    if ($data['periode'] == 'prh') {
        $sql = 'SELECT * FROM surat WHERE surKategori=? AND surJenis=? AND surTglTerimaKeluar >= ? AND surTglTerimaKeluar<=? ORDER BY surTglTerimaKeluar ASC';
        $where = array('K', 'RAHASIA', $ci->Konfigurasi->dateToMysql($data['tglawal']), $ci->Konfigurasi->dateToMysql($data['tglawal']));
        $sublabel = 'Periode ' . (($data['tglawal'] == $data['tglakhir']) ? $ci->Konfigurasi->MSQLDateToNormal($data['tglawal']) : $ci->Konfigurasi->MSQLDateToNormal($data['tglawal']) . ' - ' . $ci->Konfigurasi->MSQLDateToNormal($data['tglakhir']));
    } else if ($data['periode'] == 'prb') {
        $sql = 'SELECT * FROM surat WHERE surKategori=? AND surJenis=? AND surTglTerimaKeluar>=? AND surTglTerimaKeluar<=? ORDER BY surTglTerimaKeluar ASC';
        $where = array('K', 'RAHASIA', $data['thnawal'] . '-' . $data['blnawal'] . '-01', $data['thnakhir'] . '-' . $data['blnakhir'] . '-' . $ci->Konfigurasi->getJmlHari($data['blnakhir'], $data['thnakhir']));
        $sublabel = 'Periode ' . (($data['blnawal'] == $data['blnakhir'] && $data['thnawal'] == $data['thnakhir']) ? $ci->Konfigurasi->getNamaBulan($data['blnawal']) . ' ' . $data['thnawal'] : $ci->Konfigurasi->getNamaBulan($data['blnawal']) . ' ' . $data['thnawal'] . ' - ' . $ci->Konfigurasi->getNamaBulan($data['blnakhir']) . ' ' . $data['thnakhir']);
    } else if ($data['periode'] == 'prt') {
        $sql = 'SELECT * FROM surat WHERE surKategori=? AND surJenis=? AND YEAR(surTglTerimaKeluar)>=? AND YEAR(surTglTerimaKeluar)<=? ORDER BY surTglTerimaKeluar ASC';
        $where = array('K', 'RAHASIA', $data['thnawal2'], $data['thnakhir2']);
        $sublabel = 'Periode Tahun ' . (($data['thnawal2'] == $data['thnakhir2']) ? $data['thnawal2'] : $data['thnawal2'] . ' - ' . $data['thnakhir2']);
    }
}
?>
<div style="font-size: 14px; font-weight: bold;" align="center">Laporan Surat <?php echo(($data['kategori'] == 'MASUK') ? 'Masuk' : 'Keluar'); ?> (Jenis : Rahasia)</div><br/>
<div style="font-size: 14px; font-weight: bold;" align="center"><?php echo $sublabel; ?></div><br/>
<table align="center">
    <tr>
        <td class="th" valign="top" align="center" width="40">No</td>
        <td class="th" valign="top" align="center" width="120">No. Urut</td>
        <td class="th" valign="top" align="center" width="110">Tanggal Terima</td>
        <td class="th" valign="top" align="center" width="70">Keamanan</td>
        <td class="th" valign="top" align="center" width="90">Sifat</td>
        <td class="th" valign="top" align="center" width="350">Asal</td>
        <td class="th" valign="top" align="center" width="350">Tujuan</td>
    </tr>
    <?php
    ...
    <tr><td colspan="7" style="border: 0px;"></td></tr>  
    <tr><td colspan="7" style="border: 0px;"></td></tr>
    <tr><td colspan="6" style="border: 0px;"></td><td style="border: 0px;" colspan="1">Pekanbaru, <?php echo $ci->Konfigurasi->MSQLDateToNormal(date('Y-m-d')); ?></td></tr>
    <tr><td colspan="6" style="border: 0px;"></td><td style="border: 0px;" colspan="1">Ketua Pengadilan Agama Pekanbaru,</td></tr>
    <tr><td colspan="7" style="border: 0px;"></td></tr>  
    <tr><td colspan="7" style="border: 0px;"></td></tr>  
    <tr><td colspan="7" style="border: 0px;"></td></tr>
    <tr><td colspan="6" style="border: 0px;"></td><td style="border: 0px;" colspan="1"><?php echo $ci->PegawaiModel->getbyid('58d76d351c75f')->pegNama; ?></td></tr>
</table>
