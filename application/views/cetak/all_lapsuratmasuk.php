...
if ($data['kategori'] == 'MASUK') {
    if ($data['periode'] == 'prh') {
        $sql = 'SELECT * FROM surat WHERE surKategori=? AND surTglTerimaKeluar >= ? AND surTglTerimaKeluar<=? ORDER BY surTglTerimaKeluar ASC';
        $where = array('M', $ci->Konfigurasi->dateToMysql($data['tglawal']), $ci->Konfigurasi->dateToMysql($data['tglakhir']));
        ...
        $sql = 'SELECT * FROM surat WHERE surKategori=? AND surTglTerimaKeluar>=? AND surTglTerimaKeluar<=? ORDER BY surTglTerimaKeluar ASC';
        $where = array('M', $data['thnawal'] . '-' . $data['blnawal'] . '-01', $data['thnakhir'] . '-' . $data['blnakhir'] . '-' . $ci->Konfigurasi->getJmlHari($data['blnakhir'], $data['thnakhir']));
        ...
        $sql = 'SELECT * FROM surat WHERE surKategori=? AND YEAR(surTglTerimaKeluar)>=? AND YEAR(surTglTerimaKeluar)<=? ORDER BY surTglTerimaKeluar ASC';
        $where = array('M', $data['thnawal2'], $data['thnakhir2']);
    ...
    if ($data['periode'] == 'prh') {
        $sql = 'SELECT * FROM surat WHERE surKategori=? AND surTglTerimaKeluar >= ? AND surTglTerimaKeluar<=? ORDER BY surTglTerimaKeluar ASC';
        $where = array('K', $ci->Konfigurasi->dateToMysql($data['tglawal']), $ci->Konfigurasi->dateToMysql($data['tglawal']));
        $sublabel = 'Periode ' . (($data['tglawal'] == $data['tglakhir']) ? $ci->Konfigurasi->MSQLDateToNormal($data['tglawal']) : $ci->Konfigurasi->MSQLDateToNormal($data['tglawal']) . ' - ' . $ci->Konfigurasi->MSQLDateToNormal($data['tglakhir']));
    } else if ($data['periode'] == 'prb') {
        $sql = 'SELECT * FROM surat WHERE surKategori=? AND surTglTerimaKeluar>=? AND surTglTerimaKeluar<=? ORDER BY surTglTerimaKeluar ASC';
        $where = array('K', $data['thnawal'] . '-' . $data['blnawal'] . '-01', $data['thnakhir'] . '-' . $data['blnakhir'] . '-' . $ci->Konfigurasi->getJmlHari($data['blnakhir'], $data['thnakhir']));
        $sublabel = 'Periode ' . (($data['blnawal'] == $data['blnakhir'] && $data['thnawal'] == $data['thnakhir']) ? $ci->Konfigurasi->getNamaBulan($data['blnawal']) . ' ' . $data['thnawal'] : $ci->Konfigurasi->getNamaBulan($data['blnawal']) . ' ' . $data['thnawal'] . ' - ' . $ci->Konfigurasi->getNamaBulan($data['blnakhir']) . ' ' . $data['thnakhir']);
    } else if ($data['periode'] == 'prt') {
        $sql = 'SELECT * FROM surat WHERE surKategori=? AND YEAR(surTglTerimaKeluar)>=? AND YEAR(surTglTerimaKeluar)<=? ORDER BY surTglTerimaKeluar ASC';
        $where = array('K', $data['thnawal2'], $data['thnakhir2']);
        $sublabel = 'Periode Tahun ' . (($data['thnawal2'] == $data['thnakhir2']) ? $data['thnawal2'] : $data['thnawal2'] . ' - ' . $data['thnakhir2']);
    }
}
?>
<div style="font-size: 14px; font-weight: bold;" align="center">Laporan Surat <?php echo(($data['kategori'] == 'MASUK') ? 'Masuk' : 'Keluar'); ?></div><br/>
...
<table align="center">
    <tr>
        ...
        <td class="th" valign="top" align="center" width="50">Jenis</td>
        <td class="th" valign="top" align="center" width="210">Perihal</td>
    ...
    if (count($surat) < 1) {
        echo '<tr><td valign="top" align="left" colspan="11" style="font-size: 14px;"><i>Data Tidak Tersedia</i></td></tr>';
    } else {
        $no = 1;
        foreach ($surat as $row) {
            ...
            <tr>
                ...
                <td valign="top" align="center"><?php echo ucwords($row->surJenis); ?></td>
