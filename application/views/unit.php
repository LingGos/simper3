...
switch ($subview) {
    default:
        ...
        <div class="row padding20">
            <h1 class="text-light fg-orange"><a href="" class="icon mif-arrow-left fg-green"><span></span></a> Kelola Unit <a class="image-button warning place-right" onclick="tambah()">Tambah Data<span class="icon mif-plus bg-darkOrange"></span></a></span></h1>
            ...
            <table class="dataTable border bordered tabel" data-role="datatable" data-auto-width="false">
                ...
                <td class="sortable-column sort-asc">Jenis</td>
                ...
                <tbody>
                    ...
                    foreach ($data_query as $row) {
                        ?>
                        <tr>
                            <td class="align-center"><a onclick="detail('<?php echo $row->uniID; ?>')" class="cycle-button mini-button success"><span class="mif-eye"></span></a>&nbsp;<a onclick="ubah('<?php echo $row->uniID; ?>')" class="cycle-button mini-button primary"><span class="mif-pencil"></span></a>&nbsp;<a onclick="hapus('<?php echo $row->uniID; ?>')" class="cycle-button mini-button alert"><span class="mif-cross"></span></a></td> 
                            <td class="align-center"><?php echo $row->uniID; ?></td>
                            <td><?php echo $row->uniNama; ?></td>
                            <td><?php echo ucwords($row->uniJenis); ?></td>
                            <td><?php echo $row->uniKet; ?></td>
                        </tr>
                        <?php
                    }
                    ?>
...
<script type="text/javascript">
                ...
                function ubah(id) {
                    ...
                    metroDialog.toggle('#modalUnit');
                    $.ajax({
                        url: "<?php echo base_url(); ?>unit/getbyid/" + id,
                        type: 'POST',
                        dataType: 'JSON',
                        canche: false,
                        success: function(data) {
                            $('[name="uniID"]').val(data.uniID);
                            $('[name="uniNama"]').val(data.uniNama);
                            $('[name="uniKet"]').val(data.uniKet);
                            if (data.uniJenis == 'INTERNAL') {
                                $('#uniJenisI').attr("checked", true);
                                $('#uniJenisE').attr("checked", false);
                            } else {
                                $('#uniJenisI').attr("checked", false);
                                $('#uniJenisE').attr("checked", true);
                            }
                ...
                function hapusdata() {
                    $.ajax({
                        url: "<?php echo base_url('unit/hapusbyid'); ?>/" + idhapus,
                        dataType: "JSON",
                        success: function(data) {
                            location.reload();
                        }, error: function(jqXHR, textStatus, errorThrown) {
...
<div data-role="dialog" id="modalUnit" class="padding20 text-bold" data-close-button="true" data-width="50%" data-show="false" data-overlay="true" data-overlay-color="op-gray" data-overlay-click-close="false" data-color="fg-green">
    <div class="row">
        <h3 class="text-light fg-orange" id="judulTUnit"><span class="icon mif-plus small bg-orange fg-white cycle-button"></span> Tambah Unit</h3>
        <h3 class="text-light fg-orange" id="judulUUnit"><span class="icon mif-pencil small bg-orange fg-white cycle-button"></span> Ubah Unit</h3>
        <h3 class="text-light fg-orange" id="judulDUnit"><span class="icon mif-list small bg-orange fg-white cycle-button"></span> Detail Unit</h3>
        <hr class="bg-orange"/>
            ...
            <div class="grid">
                ...
                <div class="row cells12">
                    ...
                    <div class="cell colspan7"><div class="input-control text success full-size">
                            <input name="uniNama" type="text" data-validate-func="required" data-validate-hint="Data Nama Tidak Boleh Kosong..!">
                            <span class="input-state-error mif-warning"></span>
                        </div></div>
                </div>
                <div class="row cells12">
                    <div class="cell colspan3"><label>Jenis </label><label class="fg-red">*</label></div>
                    <div class="cell colspan2">
                        <div class="input-control text success full-size">
                            <label class="input-control radio small-check">
                                <input type="radio" name="uniJenis" value="INTERNAL" id="uniJenisI" checked>
                    ...
                    <div class="cell colspan5">
                        <div class="input-control text success full-size">
                            <label class="input-control radio small-check">
                                <input type="radio" value="EKSTERNAL" id="uniJenisE" name="uniJenis">
                                <span class="check"></span>
                                <span class="caption">Eksternal</span>
                            </label>
                        </div>
                    </div>
                </div>
