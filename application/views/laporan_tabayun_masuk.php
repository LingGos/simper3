...
switch ($subview) {
    case 'cetakchart':
        ...
        <div class="row padding20">
            <h1 class="text-light fg-orange"><a href="" class="icon mif-arrow-left fg-green"><span></span></a> Chart Diagram Surat Masuk </h1>
            ...
            if ($data['js'] == 'ALL') {
                echo '<center><h3>Diagram Jumlah Tabayun Masuk Semua Jurusita/Jurusita Pengganti</h3></center>';
                if ($data['periode'] == 'prh') {
                    //select js/jsp aktif habis tu select berdasarkan id js/jsp yg di goup by pertgl
                    ...
                    $where_all = array('M', $ci->Konfigurasi->dateToMysql($data['tglawal']), $ci->Konfigurasi->dateToMysql($data['tglakhir']));
                    ...
                    $where_biasa = array('M', 'BIASA', $ci->Konfigurasi->dateToMysql($data['tglawal']), $ci->Konfigurasi->dateToMysql($data['tglakhir']));
                    ...
                    $where_rahasia = array('M', 'RAHASIA', $ci->Konfigurasi->dateToMysql($data['tglawal']), $ci->Konfigurasi->dateToMysql($data['tglakhir']));
                    ...
                    $where_all = array('M', $data['thnawal'] . '-' . $data['blnawal'] . '-01', $data['thnakhir'] . '-' . $data['blnakhir'] . '-' . $ci->Konfigurasi->getJmlHari($data['blnakhir'], $data['thnakhir']));
                    ...
                    $where_penting = array('M', 'PENTING', $data['thnawal'] . '-' . $data['blnawal'] . '-01', $data['thnakhir'] . '-' . $data['blnakhir'] . '-' . $ci->Konfigurasi->getJmlHari($data['blnakhir'], $data['thnakhir']));
                    ...
                    $where_biasa = array('M', 'BIASA', $data['thnawal'] . '-' . $data['blnawal'] . '-01', $data['thnakhir'] . '-' . $data['blnakhir'] . '-' . $ci->Konfigurasi->getJmlHari($data['blnakhir'], $data['thnakhir']));
                    ...
                    $where_rahasia = array('M', 'RAHASIA', $data['thnawal'] . '-' . $data['blnawal'] . '-01', $data['thnakhir'] . '-' . $data['blnakhir'] . '-' . $ci->Konfigurasi->getJmlHari($data['blnakhir'], $data['thnakhir']));
                    ...
                    $where_all = array('M', $data['thnawal2'], $data['thnakhir2']);
                    ...
                    $where_penting = array('M', 'PENTING', $data['thnawal2'], $data['thnakhir2']);
                    ...
                    $where_biasa = array('M', 'BIASA', $data['thnawal2'], $data['thnakhir2']);
                    ...
                    $where_rahasia = array('M', 'RAHASIA', $data['thnawal2'], $data['thnakhir2']);
                ...
                echo '<center><h3>Diagram Jumlah Surat Masuk (Jenis : Penting)</h3></center>';
                if ($data['periode'] == 'prh') {
                    $sql_penting = 'SELECT surTglTerimaKeluar,COUNT(*) AS surJml FROM surat WHERE surKategori=? AND surJenis=? AND surTglTerimaKeluar >= ? AND surTglTerimaKeluar<=? GROUP BY surTglTerimaKeluar ORDER BY surTglTerimaKeluar ASC';
                    $where_penting = array('M', 'PENTING', $ci->Konfigurasi->dateToMysql($data['tglawal']), $ci->Konfigurasi->dateToMysql($data['tglakhir']));
                    $sublabel = 'Periode ' . (($data['tglawal'] == $data['tglakhir']) ? $ci->Konfigurasi->MSQLDateToNormal($data['tglawal']) : $ci->Konfigurasi->MSQLDateToNormal($data['tglawal']) . ' - ' . $ci->Konfigurasi->MSQLDateToNormal($data['tglakhir']));
    ...
    default:
        ?>
        <div class="row padding20">
            <h1 class="text-light fg-orange"><a href="" class="icon mif-arrow-left fg-green"><span></span></a> Laporan Surat Masuk </h1>
            <hr class="thin bg-orange">
            <?php if ($this->session->flashdata("msg_sukses") != "") { ?>
                <div class="row" id="notif">
                    <div class="padding10 bg-green fg-white text-light"><span class="mif-warning"></span> <?php echo $this->session->flashdata("msg_sukses"); ?></div>
            ...
            <div class="row grid">
                <div class="row cells12 no-margin">
                    <div class="cell colspan2 bg-green fg-white padding10" style="margin-top: 5px;"><label>Jenis Laporan </label><label class="fg-red">*</label></div>
                ...
                <div class="row cells12 no-margin">
                    <div class="cell colspan2 bg-green fg-white padding10" style="margin-top: 5px;"><label>Jurusita </label><label class="fg-red">*</label></div>
                    <div class="cell colspan4">
                        <div class="input-control success full-size">
                            <select class="js-select full-size" name="lapJs">
                                <option value="ALL">Semua</option>
                                <?php
                                $data=$ci->LaporanModel->getDataQuery("SELECT * FROM pegawai WHERE pegUniID=?",array(config_item("ID_JABJS/JSP_AKTIF")));
                                foreach ($data as $v) {
                                    echo '<option value="'.$v->pegID.'">'.$v->pegNip.' | '.$v->pegNama.'</option>';
                                }
                                ?>
                            </select>
                        </div>
    ...
    $(document).ready(function() {
        ...
        function validate(ref) {
            ...
            if (res) {
                ...
                window.open(ref + "/" + jenis + "/" + js + "/" + periode + "/" + lapTglAwal + "/" + lapTglAkhir + "/" + lapBulanAwal + "/" + lapBulanAkhir + "/" + lapTahunAwal + "/" + lapTahunAkhir + "/" + lapTahun2Awal + "/" + lapTahun2Akhir);
            }
        }
